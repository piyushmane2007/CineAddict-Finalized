    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            setError(err.response?.data?.message ||
                ...
                'Failed to register account. Please check your network or Flask backend.');
        }
        finally {
    ...
    return (<div className="min-h-[80vh] flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-md w-full bg-[#131927]/90 backdrop-blur-xl border border-white/10 rounded-3xl p-8 shadow-2xl shadow-purple-950/20 space-y-6">
        <div className="text-center space-y-2">
          ...
          <h2 className="text-2xl font-black text-white tracking-tight">Create CineAddict Account</h2>
          <p className="text-xs text-purple-200/80">Join the movie enthusiast community today</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-1">
            ...
            <div className="relative">
              ...
              <input type="text" value={username} onChange={(e) => setUsername(e.target.value)} required placeholder="cinephile99" className="w-full pl-11 pr-4 py-3 bg-slate-950/80 border border-white/10 rounded-xl text-white text-sm focus:outline-none focus:border-purple-500 transition-colors"/>
            </div>
          </div>
